"""
This File reads the IronOre price csv and writes the oil price expect scenario
...
year = 20
filepath = '/Users/itokoudai/Documents/school_class/MyLabratory/GraduateThesis/code/data/ODA-PIORECR_USD.csv'
kind = "IronOre"
...
def binomial_lattice_model():
	...
	for num in range(year * 12):
		...
		IronOre_price_scenario.append(next_price)
		if now_month == 13:
			now_month = 1
			now_year += 1
		csvWriter.writerow([(str(now_year) + '/' + str(now_month)), next_price])
	f.close()
	return IronOre_price_scenario
